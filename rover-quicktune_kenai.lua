--[[
moded for Kenai auto active on CYRCLE and stoped by mode chgande to anything else. Auto save if finished succesfuly.
heap size 65536 to run and mode lua needs to be deactivated mav FTP ei not work but it is posible tu run it!
...
local CIRCLE_MODE_NUMBER = 9        -- Rover Mode::Number::CIRCLE - verify against your Rover/mode.h before use  
local was_in_circle = false         -- tracks previous-tick mode state to detect entry/exit edges
...
function update()
  ...
  local in_circle = (vehicle:get_mode() == CIRCLE_MODE_NUMBER)  
  
  -- get output throttle  
  local _, throttle_out = vehicle:get_steering_and_throttle()  
  
  if in_circle and (not arming:is_armed() or (throttle_out <= 0)) and get_time() > last_warning + 5 then  
    gcs:send_text(MAV_SEVERITY.CRITICAL, "RTun: must be armed and moving to tune")  
    last_warning = get_time()  
    return  
  end  
  
  if not in_circle or not arming:is_armed() then  
    -- left Circle mode (or disarmed) since last tick: finalize  
    if was_in_circle then  
      if need_restore then  
        need_restore = false  
        if tune_done_time ~= nil then  
          save_all_params()  
          gcs:send_text(MAV_SEVERITY.NOTICE, "RTun: left Circle, tune complete -> gains saved")  
        else  
          restore_all_params()  
          gcs:send_text(MAV_SEVERITY.CRITICAL, "RTun: left Circle before done -> gains reverted")  
        end  
