...
-- Nano companion-computer heartbeat watchdog (Telem2, compid=191)  
...
local function update()  
 ...
 if connected and now-last_valid_pkt>TIMEOUT then   
  ...
  -- Nano companion-computer watchdog: HOLD + DISARM if its heartbeat goes stale  
  if nano_connected and now-last_nano_seen>NANO_TIMEOUT_MS then  
    nano_connected=false  
    if armed then  
      vehicle:set_mode(4)  
      if arming:disarm() then  
        gcs:send_text(2,"Nano: offline -> HOLD + DISARM")  
      else  
        gcs:send_text(3,"Nano: offline HOLD")  
      end  
    else  
      gcs:send_text(3,"Nano: offline")  
    end  
  end  
